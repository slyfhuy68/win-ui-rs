...
impl Into<WINDOW_STYLE> for ComboBoxStyle {
    fn into(self) -> WINDOW_STYLE {
        ...
        match self.show_type {
            ...
            EditLike {auto_scroll, always_show} => {
            	if auto_scroll {
...
impl ComboBox {
    ...
    ///搜索不区分大小写    
    pub fn element_offset_start(&self, start: ListBoxItemPos, text: &str) -> Result<Option<ListBoxItemPos>>{
        let (text_ptr, _text_u16) = str_to_pcwstr(text);
        match unsafe {SendMessageW(
                self.0.handle(),
                CB_FINDSTRING,
    ...
    ///搜索不区分大小写    
    pub fn element_offset(&self, start: ListBoxItemPos, text: &str) -> Result<Option<ListBoxItemPos>>{
        let (text_ptr, _text_u16) = str_to_pcwstr(text);
        match unsafe {
            SendMessageW(
                self.0.handle(),
